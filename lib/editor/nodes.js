...
export function* nodes(editor, options = {}) {
    const { at = editor.selection, mode = 'all', universal = false, reverse = false, voids = false, ignoreNonSelectable = false, } = options;
    let { match } = options;
    if (!match) {
        match = () => true;
    }
